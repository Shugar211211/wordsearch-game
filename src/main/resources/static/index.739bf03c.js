...
submitWordBtn.addEventListener("click", async ()=>{
    ...
    if (wordLidtSection.lastChild) wordLidtSection.removeChild(wordLidtSection.lastChild);
...
class Grid {
    ...
    getCellsInRange(firstLetter, currentLetter) {
        ...
        // right direction
        if (deltaY === 0 && deltaX > 0) for(let i = firstLetter.x; i <= currentLetter.x; i++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${i}"][data-y="${currentLetter.y}"]`));
        else if (deltaY === 0 && deltaX < 0) for(let i1 = firstLetter.x; i1 >= currentLetter.x; i1--)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${i1}"][data-y="${currentLetter.y}"]`));
        else if (deltaX === 0 && deltaY > 0) for(let i2 = firstLetter.y; i2 <= currentLetter.y; i2++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${currentLetter.x}"][data-y="${i2}"]`));
        else if (deltaX === 0 && deltaY < 0) for(let i3 = firstLetter.y; i3 >= currentLetter.y; i3--)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${currentLetter.x}"][data-y="${i3}"]`));
        else if (deltaY > 0 && deltaX > 0) for(let i4 = 0; i4 <= deltaX; i4++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${firstLetter.x + i4}"][data-y="${firstLetter.y + i4}"]`));
        else if (deltaY < 0 && deltaX < 0) for(let i5 = 0; i5 <= Math.abs(deltaY); i5++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${firstLetter.x - i5}"][data-y="${firstLetter.y - i5}"]`));
        else if (deltaY > 0 && deltaX < 0) for(let i6 = 0; i6 <= deltaY; i6++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${firstLetter.x - i6}"][data-y="${firstLetter.y + i6}"]`));
        else if (deltaY < 0 && deltaX > 0) for(let i7 = 0; i7 <= deltaX; i7++)cellsInRange.push(this.gridArea.querySelector(`td[data-x="${firstLetter.x + i7}"][data-y="${firstLetter.y - i7}"]`));
        return cellsInRange;
    }
    ...
    renderGrid(gridSize, wordGrid) {
        ...
        if (this.gridArea.lastChild) this.gridArea.removeChild(this.gridArea.lastChild);
        ...
        tbl.addEventListener("mouseup", (event)=>{
            ...
            else if (this.words.indexOf(reverseSelectedWord) !== -1) this.foundWords.push(reverseSelectedWord);
